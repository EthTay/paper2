...
#sims = list(set(glob(path + f'Nbody6_sims/{suite}/Halo*')) - set(glob(path + f'Nbody6_sims/{suite}/Halo*M*')))
#sims = list(set(glob(path + f'Nbody6_sims/{suite}/Halo*M*')))
#data = np.genfromtxt(path + f'scripts/files/GC_property_table_CHIMERA_massive.txt', unpack=True, skip_header=2, dtype=None)
#sims = ['/vol/ph/astro_data/shared/morkney2/GC_mock_obs/Nbody6_sims/Full_compact/Halo383_Massive_output_00036_129']
sims = list(set(sims) - set(['/vol/ph/astro_data/shared/morkney2/GC_mock_obs/Nbody6_sims/Full/Halo383_fiducial_early_output_00026_1009']))
...
#sims = ['/vol/ph/astro_data/shared/morkney2/GC_mock_obs/Nbody6_sims/Full/Halo383_fiducial_early_output_00021_13']
for sim in sims:
  ...
  elif GC_data[sim.split('/')[-1]]['rg'][-1] > 10e3:
    print('>    Unbound entry.')
    continue
  ...
  rate_of_change = np.abs(np.gradient(np.gradient(GC_data[sim]['rg'][not_nan])))
  weights = 1 / rate_of_change
  xspl = splrep(orig_res, GC_data[sim]['posg'][not_nan,0], w=weights, s=3)
  xorb = splev(spline_res, xspl)
  yspl = splrep(orig_res, GC_data[sim]['posg'][not_nan,1], w=weights, s=3)
  yorb = splev(spline_res, yspl)
  zspl = splrep(orig_res, GC_data[sim]['posg'][not_nan,2], w=weights, s=3)
  zorb = splev(spline_res, zspl)
  rorb = np.linalg.norm([xorb, yorb, zorb], axis=0)

  # Find the apo- and pericentres:
  differential = np.diff(np.sign(np.diff(rorb)))
  peris = np.where(differential > 0)[0] + 2
  apos = np.where(differential < 0)[0] + 2
