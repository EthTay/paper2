...
if [[ $suite == *"compact"* ]]; then
  ...
  IC_path="../Nbody6_sims/GC_ICs_Raw/"
else
...
# List mode:
#--------------------------------------------------------------------------
#list=("../Nbody6_sims/${suite}_files/Halo1459_fiducial_hires_output_00020_2.txt")
#for file in "${list[@]}"; do
for file in ../Nbody6_sims/${suite}_files/Halo*; do
  ...
  if [ -f ${IC_path}/${path}/fort.10 ]; then
    echo "!!!WARNING!!! Duplicating GC ICs from stored file."
  ...
  if [[ $SIM == *"Raw"* ]]; then SIM=${SIM/Raw/Full}; fi
  SIM2=${SIM%"_compact"}
  cp ./files/$SIM2.txt $directory/Host.txt
