...
# Parse GC properties:
host_sim = data[GC_ID]['sim']
GC_pos = np.array(data[GC_ID]['Galacto-centred position'])
GC_vel = np.array(data[GC_ID]['Galacto-centred velocity'])
GC_hlr = data[GC_ID]['3D half-mass radius']
GC_mass = data[GC_ID]['Masses'].sum()
GC_Z = data[GC_ID]['Median Fe/H']
GC_birthtime = data[GC_ID]['Median birthtime']

# Correct the EDGE outputs for the case of Halo383_Massive:
if 'Massive' in host_sim:
  EDGE_output = 'output_%s' % data[GC_ID]['Output Number']
else:
  EDGE_output = 'output_%s' % data[GC_ID]['Output Number']

EDGE_sim_name = host_sim
EDGE_halo = data[GC_ID]['Tangos Halo ID'] + 1
internal_ID = GC_ID + 1000

# Parse particle properties:
GC_masses = data[GC_ID]['Masses']
GC_metals = data[GC_ID]['Fe/H Values']
GC_births = data[GC_ID]['Birth Times']
#--------------------------------------------------------------------------
