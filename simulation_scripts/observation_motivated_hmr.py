...
mass_bins = np.logspace(np.log10(2e3), np.log10(4e4), N_bins)
...
def get_dict(key, d=data):
  return np.concatenate([[d[i][j][key] for j in list(d[i].keys()) if j in data[i].keys()] for i in list(d.keys())])

# Parse GC properties:
GC_pos = get_dict('Galacto-centred position')
GC_vel = get_dict('Galacto-centred velocity')
GC_hlr = get_dict('3D half-mass radius', d=orig_data)
GC_mass = np.array([np.sum(i) for i in get_dict('Stellar Mass', d=orig_data)])
GC_Z = get_dict('Median Fe/H', d=orig_data)
...
#ax[0].set_xlim(xmax=5e5)

line = np.array([ax[0].get_xlim()[0], 1e5, ax[0].get_xlim()[1]])
line_mass = young_SC_fit(line)
ax[0].plot(line, line_mass, 'r-', lw=1)
line_plus = np.exp(np.log(line_mass)+0.735)
ax[0].plot(line, line_plus, 'r--', lw=1)
line_minus = np.exp(np.log(line_mass)-0.735)
ax[0].plot(line, line_minus, 'r--', lw=1)
